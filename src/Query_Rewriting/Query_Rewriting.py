...
class QueryRewriter:
    def __init__(self, ollama_url="http://localhost:11434/api/chat", model_name="llama3.2"):
        '''
        Se define la clase QueryRewriter, la cual se encarga de reescribir una consulta en inglés de manera 
        concisa y sin contexto adicional.

        Args:

        ollama_url: URL del servidor de Ollama. 
        model_name: Nombre del modelo de lenguaje a utilizar para la reescritura.
    ...
    def rewrite(self, query):
        '''
        Se define la funcion rewqrite, la cual reescribe una consulta en inglés de manera concisa 
        ...
        try:
            response = requests.post(self.ollama_url, json=payload, stream=True)                # strea=True para obtener la respuesta línea por línea
